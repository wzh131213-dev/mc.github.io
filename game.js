...
class WorldManager {
  ...
  setVoxel(x, y, z, type) {
    ...
    if (!chunk) {
      chunk = new Uint8Array(CHUNK_SIZE_X * CHUNK_SIZE_Y * CHUNK_SIZE_Z);
...
class PlayerEntity {
  ...
  update(delta, inputMoveVector) {
    ...
    this.velocity.x = inputMoveVector.x;
    this.velocity.z = inputMoveVector.z;
...
function renderHotbar() {
  ...
  hotbarSlots.forEach((id, slotIndex) => {
    ...
    if (!def || (def.creativeOnly && gameMode !== 'creative')) return;
    ...
    if (def) {
      ...
      icon.style.backgroundColor = `rgb(${def.color.join(',')})`;
...
function renderInventoryUI() {
  ...
  if (hotbarGrid) {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      slot.addEventListener('click', () => handleInventorySlotClick(id, 'hotbar', i));
  ...
  if (recipesList) {
    ...
    basicRecipes.forEach(r => {
      ...
      if (gameMode !== 'creative') {
        for (let id in r.cost) {
          if ((inventory[id] || 0) < r.cost[id]) { canCraft = false; break; }
        }
      }
...
function handleInventorySlotClick(clickedId, targetType, targetIndex) {
  if (!heldItem) {
    if (targetType === 'backpack') {
      ...
      if (idInSlot) heldItem = { id: idInSlot };
    }
  } else {
    if (targetType === 'hotbar') {
      hotbarSlots[targetIndex] = heldItem.id;
      selectedBlockId = heldItem.id;
    } else if (targetType === 'backpack') {
      if (clickedId) selectedBlockId = clickedId;
    }
    heldItem = null;
  }
  renderInventoryUI();
  ...
  if (hitResult.hit) {
    ...
    if (placeDef && !placeDef.isItem) {
      if (gameMode === 'creative' || (inventory[selectedBlockId] && inventory[selectedBlockId] > 0)) {
        const targetAABB = {
          minX: hitResult.px, maxX: hitResult.px + 1,
          minY: hitResult.py, maxY: hitResult.py + 1,
          minZ: hitResult.pz, maxZ: hitResult.pz + 1
        };
        const playerAABB = player.getAABB();
        const overlap = !(targetAABB.maxX <= playerAABB.minX || targetAABB.minX >= playerAABB.maxX ||
                          targetAABB.maxY <= playerAABB.minY || targetAABB.minY >= playerAABB.maxY ||
                          targetAABB.maxZ <= playerAABB.minZ || targetAABB.minZ >= playerAABB.maxZ);
        if (!overlap) {
...
function updateDayNightCycle(delta) {
  ...
  if (dayTime < old) { survivalDays++; const dv = document.getElementById('day-count-val'); if (dv) dv.innerText = survivalDays; }
  ...
  if (gameMode === 'survival') {
    ...
    if (isNight && now - lastSpawnTime > 7000) {
      ...
      if (mobs.length < 15) {
        ...
        mob.position.set(sx, sy + 2, sz);
        mob.userData = { hp: 3, lastAttack: 0 };
        scene.add(mob);
        mobs.push(mob);
      }
    }
    ...
    for (let i = mobs.length - 1; i >= 0; i--) {
      ...
      if (dist < 1.3 && performance.now() - m.userData.lastAttack > 1200) {
        m.userData.lastAttack = performance.now();
        takeDamage(2);
      }
    }
  }
...
function updateHealthUI() {
  const bar = document.getElementById('health-bar'); if (!bar) return;
  bar.innerHTML = '';
  const hearts = Math.ceil(hp / 2);
  for (let i = 0; i < MAX_HP / 2; i++) {
    const h = document.createElement('span');
    h.className = 'heart-unit';
    h.innerText = i < hearts ? '❤️' : '🖤';
    bar.appendChild(h);
  }
}
...
function updateHungerUI() {
  ...
  const text = document.getElementById('hunger-val-text');
  const pct = (hunger / MAX_HUNGER) * 100;
  if (fill) fill.style.width = pct + '%';
  if (text) text.innerText = `${hunger}/${MAX_HUNGER}`;
}
...
function consumeSelectedFood() {
  ...
  starvationTimer = 0; hungerTimer = 0;
  updateHungerUI(); updateHealthUI(); renderHotbar(); renderInventoryUI();
}
...
function animate() {
  ...
  if (isInGame && player) {
    ...
    if (joystickMove.x !== 0 || joystickMove.y !== 0) {
      moveVec.x = joystickMove.x;
      moveVec.z = joystickMove.y;
    }
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (/^Digit[1-9]$/.test(e.code)) {
      ...
      if (slot >= 0 && slot < hotbarSlots.length) {
        selectedBlockId = hotbarSlots[slot];
        renderHotbar();
      }
    }
    ...
    window.addEventListener('mousedown', (e) => {
      if (isInGame && !isInventoryOpen && !isCraftingTableOpen) {
        if (document.pointerLockElement !== document.body) document.body.requestPointerLock?.();
          ...
          if (e.button === 2) handleBlockPlace();
        }
      }
    });
...
function startGame(mode) {
  ...
  const status = document.getElementById('status-container'); if (status) status.style.display = mode === 'survival' ? 'flex' : 'none';
